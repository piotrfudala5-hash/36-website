...
function getFriendlyLoginError(error) {
  ...
  if (code.includes('popup-blocked') || lowerMessage.includes('popup blocked')) {
    return 'Przegl�darka zablokowa�a okno logowania Google. Zezw�l na popupy dla tej strony.';
  }

  if (code.includes('popup-closed-by-user')) {
    return 'Okno logowania zosta�o zamkni�te przed zako�czeniem. Spr�buj ponownie.';
  }

  if (code.includes('unauthorized-domain') || lowerMessage.includes('unauthorized-domain')) {
    return 'Domena nieautoryzowana w Firebase Auth. Dodaj domen� strony do Authorized domains.';
  }

  if (code.includes('operation-not-allowed') || lowerMessage.includes('operation-not-allowed')) {
    return 'Google Sign-In jest wy��czony w Firebase Authentication. W��cz metod� logowania Google.';
  }

  return message || 'Logowanie nie powiod�o si�.';
}
...
loginButton.addEventListener('click', async () => {
  ...
  const fallbackTimer = setTimeout(async () => {
    ...
    loginError.hidden = false;
    loginError.textContent = 'Popup nie otworzy� si�. Uruchamiam logowanie przez przekierowanie...';
  ...
  try {
    ...
    if (!redirectStarted && shouldFallbackToRedirect(error)) {
      redirectStarted = true;
      loginError.hidden = false;
      loginError.textContent = 'Popup zablokowany. Przekierowuj� do logowania Google...';
